...
func (r *Router) initAuthRouter() {
	...
	lh := authhandler.NewRouteLoginHandler(r.db, r.log, r.env)
	rh := authhandler.NewRouteRegisterHandler(r.db, r.log, r.qr, r.env)
	vah := authhandler.NewVerifyAccountHandler(r.db, r.log, r.env)
	fh := authhandler.NewRouteRefreshHandler(r.db, r.log, r.env)
	...
	authRouter.Post("/refresh", fh.Refresh)
